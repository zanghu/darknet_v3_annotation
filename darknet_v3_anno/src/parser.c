...
/**
 * 按顺序解析配置文件逐层创建网络时, 用于将前一层参数传递给后一层
 */
typedef struct size_params{
    int batch;      // net输入batch的大小(小batch)
    int inputs;     // 前一层输出神经元个数(即单个输入样本的 输出向量维数)
    int h;          // 前一层的输出feature map高度
    int w;          // 前一层的输出feature map宽度
    int c;          // 前一层的输出feature map个数
    int index;      // 指向当前(正在创建的)层创建完成后在数组net->layers中的索引号一致. shortcut层创建过程中有用到
    int time_steps; // RNN相关层的参数
    network *net;
} size_params;
...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    int batch_normalize = option_find_int_quiet(options, "batch_normalize", 0); // yolov3.cfg中共有75个[convolutional], 其中72个batch_normalize为1, 剩余3个没有设置采用默认值0
...
int *parse_yolo_mask(char *a, int *num)
{
    ...
    if(a){
        ...
        for(i = 0; i < n; ++i){
            int val = atoi(a); // 经过测试, atoi()每次遇到','会认为是结束符
            mask[i] = val;
            a = strchr(a, ',')+1;
        }
        *num = n;
...
{
    int classes = option_find_int(options, "classes", 20); // pascal voc: 20, coco: 80, 这里与faster-rcnn有所区别, 即没有将"其他"单独列为一类
    int total = option_find_int(options, "num", 1); // 注意: total值其实来自配置文件的"num"项, num的值则接下来会被改为与"mask"项值中整数的个数相同
    int num = total;

    char *a = option_find_str(options, "mask", 0);
    // 注意接下来num被修改为与mask数组元素数相等
    int *mask = parse_yolo_mask(a, &num); // 指向int数组, 元素数等于cfg中"mask"项值中被逗号分割的数字个数, 例如"0,1,2"
...
/**
 * @brief 解析配置文件中[shortcut]部分, 创建shortcut层对象
 * @note shortcut层可以将来接收自两个不同层的输入, 理论上N个shortcut层可以将N+1个层的输入汇集在一起
 */
layer parse_shortcut(list *options, size_params params, network *net)
{
    char *l = option_find(options, "from"); // 找到额外输入层, from的值一般是负数
    int index = atoi(l);
    ...
    s.alpha = option_find_float_quiet(options, "alpha", 1); // 前一层输出的权重系数
    s.beta = option_find_float_quiet(options, "beta", 1);   // from层输出的权重系数
    return s;
}
...
/**
 * @brief 上下采样层, 根据配置参数不同, 可以分别实现对输入feature map的上采样和下采样
 */ 
layer parse_upsample(list *options, size_params params, network *net)
{
    ...
    // l.scale的作用是在对采样前后的feature map像素值乘以一个倍率l.scale
    l.scale = option_find_float_quiet(options, "scale", 1); // yolov3.cfg中[upsample]部分均未出现此项, 使用默认值1
    return l;
}
...
/**
 * @brief 解析网络配置文件中的一段[net]的内容, 利用其中信息初始化network
 *
 * @param options 指向网络配置文件[net]段内容的list对象, 每个元素是[net]段中的一个有效行
 * @param net     network对象, 将会被[net]段内容初始化
 */
void parse_net_options(list *options, network *net)
{
    // 下面一段是在设置net->batch和net->subdivs属性
    // 网络配置文件的[net]段的"batch"项配置的是一个big batch的样本数，即单个网络模型每隔big batch个样本更新一次参数
    // net->batch属性的值都是small batch的样本数, 即单个网络模型进行"一次正向传递 + 一次反向传播"的batch样本数
    net->batch = option_find_int(options, "batch",1);
    int subdivs = option_find_int(options, "subdivisions",1);
    net->time_steps = option_find_int_quiet(options, "time_steps",1); // 似乎只有rnn中有用
    net->batch /= subdivs;
...
/**
 * @brief 解析网络配置文件, 按照配置文件创建和初始化network对象
 */
network *parse_network_cfg(char *filename)
{
    list *sections = read_cfg(filename); // 解析网络配置文件, 返回一个"sections -- lines"的二级嵌套list
    ...
    // 使用网络配置文件中第一部分[net]中的信息初始化network对象
    section *s = (section *)n->val;
    list *options = s->options;
    if(!is_network(s)) error("First section must be [net] or [network]");
    parse_net_options(options, net); // 解析[net]部分的核心函数
    ...
    while(n){
        // 在shorcut层中, 将用来在net->layers中找到第二个输入层(第一个输入层是前一层)
        params.index = count; // params.index指向当前层的索引号, 该值与当前层在数组net->layers中的索引号一致.
    ...
    net->input = calloc(net->inputs*net->batch, sizeof(float)); // net->input是network专有的动态内存空间, 不是引用其他层属性的
    net->truth = calloc(net->truths*net->batch, sizeof(float)); // net->truth是network专有的动态内存空间, 不是引用其他层属性的
#ifdef GPU
...
/**
 * @brief 读取符合网络darknet配置文件格式的文件, 将其内容转化成二层嵌套的list
 *
 * typedef struct{
 *   char *type;
 *   list *options;
 ...
 *
 * 文件内容解析成一个list对象options, 每个元素是一个section对象
 * 每个secton对象的options属性也是一个list, 每个元素是一个key-val型的node, 对应配置文件中该section的一行
 * 这样就形成了"sections——lines"的二级嵌套 list结构
 *
 * @param filename 网络配置文件
 */
list *read_cfg(char *filename)
{
    ...
    while((line=fgetl(file)) != 0){
        ++ nu;
        strip(line); // 去掉字符串s中所有空格、'\t'和'\n', 结果仍保留在原字符串中(原字符串被修改). utils.c
        switch(line[0]){
            case '[':
            ...
            case '#':
            case ';':
                free(line); // 以'\0'或'#'或';'开头的行的统一处理: 释放改行内存资源，忽略之
                break;
            default:
                // read_option()返回0表示失败, 返回1表示成功, 这不符合C语言惯例
                if(!read_option(line, current->options)){ // 将一个配置行的内容转化为key-value, 插入current->options链表中. option_list.c
